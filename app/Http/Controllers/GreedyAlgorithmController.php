...
class GreedyAlgorithmController extends Controller
{
    ...
    {
        $program = 1;
        $randNumCourses = rand(6,10);
        $coursesRange = range(1,15);
        shuffle($coursesRange);
        $aY = 1;
        $semester = 1;


        for($count = 1;$count <= $randNumCourses;$count++)
        {
            $randomCourses  = $coursesRange[$count];
            $randTeacher = rand(1,10);
            $course = Course::select('title')
            ->where('id',$randomCourses)
            ->first();

            $teacher = Teacher::select(DB::raw("CONCAT(first_name,' ',last_name) AS fullName"))
            ->where('id',$randTeacher)
            ->first();

            $teachersAvailability = AvailableTime::where('teacher_id',$randTeacher)
            ->count();

            if($teachersAvailability > 3)
            {
                $teachersAvailability = 3;
            }
            
            $courses[]=$course->title;
            $teachers[]=$teacher->fullName;
            $meetings[]=rand(1,$teachersAvailability);
            $totalHours[]=rand(2,5);
            $hoursPerDay = [];
            $roomType[] = rand(1,2);
                                            ...
                                            if ($timeStart[$k] >= $room->available_time_start && $timeFinish <= $room->available_time_finish )
                                                {
                                                    ...
                                                    if($conflict == 0)
                                                    {
                                                        ...
                                                        $sched = new Schedule();
                                                        $sched->academic_year_id = $aY;
                                                        $sched->semester = $semester;
                                                        $sched->program_id = $program;
                                                        $sched->course_id = $course->id;
                                                        $sched->teacher_id = $teacherId;
                                                        $sched->room_id = $selectedRoom;
                                                        $sched->day_of_week = $selectedDay->available_day;
                                                        $sched->time_start = $timeStart[$k];
                                                        $sched->time_finish = $timeFinish;

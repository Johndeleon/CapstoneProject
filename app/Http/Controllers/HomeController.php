...
//@created by John, @since November 13
  ...
  public function getDashboard() {
      ...
      if (count($academicYears) > 0) {
          ...
          $rooms = Room::where('deleted_at', null)
                         ->get();
  ...
  /* DASHBOARD MODULE */
  public function getEmptySchedule(Request $req) {
    $p = Program::where('deleted_at', null)
                ->get();
    ...
    if (count($p) > 0) {
      ...
      foreach($p as $item) {
        ...
        for ($i = 1; $i <= $item->levels; $i++) {
          $checker = Schedule::where('program_id', $item->id)
                             ->where('level', $i)
  ...
  public function deleteSchedule(Request $req) {
    ...
    $s = Schedule::where('academic_year_id', $req->ay)
                 ->where('semester', $req->sem)
                 ->where('program_id', $req->prog)
                 ->where('level', $req->lev)
                 ->get();

    foreach ($s as $item) {
      $sdel = Schedule::where('id', $item->id)
                      ->first();
    ...
    /* Delete from fixed schedule table */
    ...
    foreach ($s as $item) {
      $sdel = FixedSchedule::where('id', $item->id)
                      ->first();
  ...
  public function getFilteredSchedule(Request $req) {
    // If rooms, program, and teacher is empty
    if ($req->rooms == null && $req->teacher == null && $req->prog == null) {
      $status = false;
      $message = "Please select atleast one of the empty data.";
    ...
    if ($req->level != null) {
      if ($req->prog == null) {
        if ($req->rooms != null) {
          ...
          if ($req->teacher != null) {
            $ts = FixedSchedule::where('teacher_name', $req->teacher)
                                  ->where('academic_year_id', $req->ay)
                                  ->where('semester', $req->sem)
                                  ->where('room_name', $req->rooms)
                                  ->get();
          
          } else { // If teacher is null 
            $ts = FixedSchedule::where('academic_year_id', $req->ay)
                                  ->where('semester', $req->sem)
                                  ->where('room_name', $req->rooms)
                                  ->get();
          }
        ...
        if (count($ts) > 0) {
          ...
          if ($req->rooms != null) {
            $message = "No schedule found for this teacher in this room.";
        ...
        if (count($ts) > 0) {
          ...
          if ($req->rooms == null && $req->teacher == null) {
            $message = "The schedule for this program is does not exist.";
          }
          else if ($req->rooms != null) {
            $message = "No schedule found for this teacher in this room.";
